...
from frappe import _, db, throw, new_doc, delete_doc, get_list

class PartCreationTool(Document):
	
	def before_submit(self):
		if self.truck_part == "Battery" or self.truck_part == "Tyre":
			if self.include_sr_no and self.number_of_parts != len(self.serial_number):
				throw(_("Please set total %d serial numbers"%self.number_of_parts))
			
		serial_list = []	
		for d in self.serial_number:
			serial = db.get_value("Truck Parts Inventory", {"serial_number": d.serial_no})
			if serial:
	...
	def on_submit(self):
		...
		parts = ["Battery", "Tyre"]
		if self.truck_part in parts and self.include_sr_no:
	...
	def on_cancel(self):
		result = get_list("Truck Parts Inventory", filters={'creation_tool_link': self.name}, fields='name')
