...
def get_log_data(filters):
	...
	data = frappe.db.sql("""
		...
		GROUP BY
			truck_no
		ORDER BY 
			total_service_bill""" % conditions, as_dict=1)
...
def get_conditions(filters):
	...
	where_clause = " and date between '%s' and '%s' " % (filters.from_date, filters.to_date)
	where_clause += filters.truck_no and " and truck_no = '%s' " % filters.truck_no.replace("'", "\'") or ""
